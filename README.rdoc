...
<tt> User.stalky.name #query all available fields in the "User" model and get most likely "name" value. If there are multiple results an array will be returned. You can ajaxify this to have the other form fields auto-fill based on the results. 
	...
	#switches
		--verbose #lists the data sources for each piece of PII </tt>
